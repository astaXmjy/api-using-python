...
def makePosts(post:schemas.CreatePost,db:Session=Depends(get_db),current_user:int=
...
def update_post(id:int,update_post:schemas.CreatePost,db:Session=Depends(get_db),current_user:int=
              Depends(oauth2.get_current_user)):
